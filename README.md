...
- **首页大卡片**：左右滑动切换主机，卡片实时显示 CPU / 内存 / 磁盘 / 网速 / 负载 / 运行时长与 CPU 走势图，点击进入详情；悬浮「＋」按钮圆形样式，列表上滑自动隐藏
...
- **设置**：日间 / 夜间 / 跟随系统；刷新间隔；生物识别锁；**备份与恢复**（口令加密，PBKDF2 + AES-256-GCM）；**应用内检测更新**（多通道代理，下载进度可视化）
